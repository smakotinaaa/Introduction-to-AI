...
class PlanGraphLevel(object):
    ...
    def update_action_layer(self, previous_proposition_layer):
        ...
        for action_to_add in all_actions:
            if not previous_proposition_layer.all_preconds_in_layer(action_to_add):
                continue
            else:
                for precondition_1, precondition_2 in itertools.combinations(action_to_add.get_pre(), 2):
                    if previous_proposition_layer.is_mutex(precondition_1,
                                                           precondition_2):
                        break
                else:
                    self.action_layer.add_action(action_to_add)
    ...
    def update_mutex_actions(self, previous_layer_mutex_proposition):
        ...
        for action_1 in current_layer_actions:
            for action_2 in current_layer_actions:
                if action_1 == action_2:
                    continue
                if mutex_actions(action_1, action_2,
                                 previous_layer_mutex_proposition):
                    self.action_layer.add_mutex_actions(action_1, action_2)
    ...
    def update_proposition_layer(self):
        ...
        Note that same proposition in different layers might have different
        producers lists, hence you should create two different instances.
        current_layer_actions is the set of all the actions in the current layer.
        You might want to use those functions:
        dict() creates a new dictionary that might help to keep track on the
        propositions that you've already added to the layer
        self.proposition_layer.add_proposition(prop) adds the proposition prop
        ...
        for action in current_layer_actions:
            for prop in action.get_add():
                if prop.get_name() in prop_dict.keys():
                    prop_dict[prop.get_name()].append(action)
                else:
                    prop_dict[prop.get_name()] = [action]
        for prop_name, prop_producers in prop_dict.items():
            new_prop = Proposition(prop_name)
            new_prop.set_producers(prop_producers)
...
def have_competing_needs(a1, a2, mutex_props):
    ...
    for a1_precondition in a1.get_pre():
        for a2_precondition in a2.get_pre():
            if Pair(a1_precondition, a2_precondition) in mutex_props:
                return True
    return False
...
def mutex_propositions(prop1, prop2, mutex_actions_list):
    ...
    You might want to use this function:
    prop1.get_producers() returns the set of all the possible actions in the
    layer that have prop1 on their add list
    """
    for a1_action in prop1.get_producers():
        for a2_action in prop2.get_producers():
            if Pair(a1_action, a2_action) not in mutex_actions_list:
                return False
    return True
