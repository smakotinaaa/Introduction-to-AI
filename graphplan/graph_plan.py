...
def independent_pair(a1, a2):
    ...
    for add_action in a1.get_add():
        if a2.is_neg_effect(add_action):
            return False
    for add_action in a2.get_add():
        if a1.is_neg_effect(add_action):
            return False

    # check interference
    for pre_cond in a1.get_pre():
        if a2.is_neg_effect(pre_cond):
            return False
    for pre_cond in a2.get_pre():
        if a1.is_neg_effect(pre_cond):
            return False
    return True
